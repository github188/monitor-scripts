...
class redis_client(redis.StrictRedis):
    def __init__(self):
        ...
        pool = redis.ConnectionPool(host = '10.0.70.76',port = 6379,db = 0,max_connections= 1)
...
def check(host,idc_name):
    ...
    paths = {
        'hy':{
            'rewriter_nodes' :'/com/wandoujia/orion/server/rewriter',
            'realtime_nodes': '/com/wandoujia/orion/leaf/realtime',
            'dataservice_nodes':'/com/wandoujia/orion/dataservice/0',
            'ssd_nodes':'/com/wandoujia/orion/leaf/ssd',
            'ssd_apps_nodes':'/com/wandoujia/mmsapps/search/leaf/ssd',
            'realtime_apps_nodes':'/com/wandoujia/mmsapps/search/leaf/realtime',
            },
        'db':{
            'segment_nodes':'/com/wandoujia/orion/db/segment',
            }
    }
    ...
    if rkh.connected:
        ...
        rewriter_nodes = [ "app626.hy01", "app625.hy01" ]
        realtime_nodes = [ "app698.hy01", "app699.hy01" ]
        dataservice_nodes = [ "app627.hy01", "app628.hy01" ]
        ssd_nodes = [ "app625.hy01", "app626.hy01" ]
        ssd_apps_nodes = [ "app18.hy01", "app21.hy01" ]
        realtime_apps_nodes = [ "app18.hy01", "app21.hy01" ]
        segment_nodes = [ "app17.db01", "app18.db01" ]
    ...
    if idc_name == "hy":
        ...
        segment_nodes = ipaddress(*segment_nodes)
    ...
    if segment_nodes:
        print "STATUS=14,segment_nodes:%s isn't connnected" % (','.join(segment_nodes.values()))
    if error_segment:
        print "STATUS=15,segment_nodes:%s wrong ip connected" % (','.join(error_segment))
    if not rewriter_nodes and not realtime_nodes and not error_rewriter and not error_realtime and not dataservice_nodes and not ssd_nodes and not error_dataservice and not error_ssd and not error_ssd_apps and not error_realtime_apps and not ssd_apps_nodes and not realtime_apps_nodes and not segment_nodes and not error_segment:
